...
Next we repeat this but restricting to forecasts made after September 1. 

```{r, fig.width = 7, fig.height = 4}
plot_err(cfr_pred, geo_value = "us", start_day = "2020-09-11") 
```
...
include 95% bands based on quantiles of residuals. In call cases, the black
...
*future* coverage. Therefore they should be looked at with skepticism when it 
comes to the actual forward-look predictions made by the CFR = 17.% forecaster 
...
```{r}
ens = covidHubUtils::load_forecasts(models = "COVIDhub-ensemble",
  ...
  select(-c(model, forecast_date, temporal_resolution, target_variable, type)) 
...
plot_err(cfr_ens_pred, geo_value = "us", start_day = "2020-09-01") 
```

Interestingly this reveals that the CFR = 1.7% forecaster is quite competitive
with the COVID Hub ensemble, over both periods (starting July 1, and starting
September 1), though only for longer lags (farther-ahead forecasts). 
