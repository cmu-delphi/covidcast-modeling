...
author: "Ryan Tibshirani, [Delphi Group](https://delphi.cmu.edu) and Sarah Colquhoun, Google"
...
as_of = "2020-12-03" # Fetch data "as of" the day we first did this analysis!
...
24 days, but has very little curvature around its maximum. The median curve is
maximized at 21 days.  These values are longer than the analysis done with with
an as of of November 16th (which were 16 and 16, respectively).  From this 
picture, it seems difficult to look at any one lag as the "right" one.
...
pretty difficult to agree on 21 vs 24 as the "right" lag with any amount 
of confidence.
...
somewhere above 1.5% (the horizontal dashed line). For all lag lengths, it looks
like the CFR is possibly dropping in early November; but it really remains to be
seen if this is a trend lasting longer than a month, especially for longer lags.
...
appears that the global CFR = 1.7% forecaster is most accurate at lag 18-20, 
that is 18 to 20 day ahead forecasts.
...
lag drifts up to somewhere close to 20-22, contributing more evidence that there
is not one "right" lag (certainly not a fixed answer throughout time). Also, the
...
(indeed they look far too narrow here). Note also that the sudden dip near the 
end of the reported deaths is likely due to Thanksgiving reporting 
irregularities.
...
Note again that the sudden dip near the end of the reported deaths is likely due
to Thanksgiving reporting irregularities.
...
particularly for 21 and 28 days ahead.  In recent weeks, the CFR-based forecast 
seems to be underpredicting as well, but the reported deaths are still within 
the 95% band.