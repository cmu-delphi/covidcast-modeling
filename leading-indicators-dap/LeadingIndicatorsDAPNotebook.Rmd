...
To better assess the possibility of using Delphi's signals to inform public health decision-making, we studied the potential of using these signals as leading indicators of significant rises in cases at the county level. In other words, the goal of this analysis is to provide greater insight into the value of Delphi’s signals in predicting future increases in cases.
...
sharp upswings in cases, something significant within a county, and of particular public health importance, as well as of practical importance for modeling and forecasting. We want to analyze the leadingness of a **signficant indicator rise** in relation to a **significant case rise**. To do this, we need to define and find periods of significant rise in these signals.
</details>

### Identifying Rises in Signals
In order to ascertain leadingness of an indicator, we want to determine whether the indicator began to rise significantly before cases began to rise significantly.

As a core component of this analysis, we need methodology to accurately identify significant rises, given a single time series of an indicator or cases. This is non-trivial, since the data is quite noisy at the county level, and clean rise/drops are rare.
...
**Best Fit Line**<br>
Another option we tried was calculating a line of best fit for the signal for fixed time periods within a larger time period. For example, calculating a line of best fit for every 21 day window within a 3 month window and choosing the period that has the highest slope as the most significant rise period in that county for that signal.
...
To identify periods of substantial increase in case or indicator signals, we estimated where signals had a positive first derivative. 
...
Blue is using smoothing spline method to estimate the derivative, Red is using local linear regression (Purple is where both methods marked the same point)
...
Final criteria for rise periods: A period is a significant rise in a smoothed signal if
  ...
  2. Each first derivative is > a certain % of other derivatives in time period (Note, for this analysis we set this to 0%, effectively not using this parameter) - *if not set to 0, it can mean the rise is a significant one for this county but also ties this decision to the specific time period we are looking at. The rise point identifications can change based on the time period if this is set greater than 0.*

  3. Period is > a certain number of days - *this means the rise is not spurious*

  4. Magnitude of increase from start to end of period is > a certain threshold (for these last two ponits, we used a thorough manual analysis to identify an ideal number of days and ideal threshold) - *this is another way to make sure that the rise is significant, not just a slight uptick in cases*
...
##### **County Selection**
To ensure our analysis captures meaningful examples where indicators lead case/death signals, we include all counties that have greater than 20 cases a day on average and indicator data for 90% of days in a given time range, and do not have zero or negative values for either cases or the indicator on any day. These criteria were selected empirically, based on visually inspecting the detected examples of case and indicator rises from our methodology.
...
After aggregating true positives, false positives, and false negatives across the entire time horizon, we calculate recall and precision as follows:  
...
*E. Limitations*  
While this method allows us to evaluate guessers to see how our model fares compared to a random guesser or a naive guesser, the recall and precision numbers do not necessarily reliably reflect the accuracy of the models. This method compares the performance of our leading indicator methodology with other guessers under particular assumptions of how leading  the indicator is generally compared to cases. In this method, we designate a number X as our ideal leadingness and inherently score situations in which the indicator rise precedes the case rise by exactly X days as more successful than if the indicator rise precedes the case rise by a day more or less. In reality we want to count as a success a window of differing lengths of leadingness. We ideally want to count an indicator rise point preceding a case rise point by 7 days as just as successful as one preceding a case rise point by 8 days, for example. Also, while most other predictive models are evaluated on unchanging ground truth, the ground truth in this method can change based on the value selected for X.
...
We count all the indicator rise points and all the case rise points in all the counties. In each county, for each indicator rise point, we add 1 to our **true positive** (or **success**) count if it is followed by a case rise point at least Y days and no more than Z days later. We sum the number of **successes** in each county. For this analysis, Y = 2 and Z = 14, as this is the established incubation period range for Covid-19.
...
As an example, we'll use our Dr Visits % CLI as our indicator. We use our LeadingIndicatorTools package for all our main functions (available in the Git repo).
...
```{r}
plot_signals(get_subset_of_time("2020-06-01", "2020-11-30", drs_visits_with_points), "40143", smooth_and_show_increase_point=FALSE, "Drs Visits")
```
<br>

#### Step 3. Mark the rise points (the points at the beginning of the rise periods) for the Drs Visits and Cases signal. 
In the respective rise point columns, the day is marked with a 1 if it is found to be a rise point for that signal. We can see here that there is a rise point for Drs Vists on 10/23/2020 and for cases on 10/27/2020.
```{r}
head(drs_visits_with_points[[399]],500)
```
<br>

#### Step 4. Plot the smoothed signal with the beginning rise points. 
We can see where the Drs Visits signal begins to rise before cases rise. 
```{r}
plot_signals(get_subset_of_time("2020-06-01", "2020-11-30", drs_visits_with_points), "40143", smooth_and_show_increase_point=TRUE, "Drs Visits")
```
...
##### Examples
We can plot some of the counties where rises in the doctor visits indicator lead rises in cases.
...
plot(as.factor(drs_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in one of the google symptoms signals (`sum_anosmia_ageusia_smoothed_search`) lead rises in cases.
...
plot(as.factor(google_symptoms_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the quidel indicator (`covid_ag_smoothed_pct_positive`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(quidel_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the FB CLI indicator (`smoothed_whh_cmnty_cli`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(fb_cli_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the FB public transit indicator (`smoothed_wpublic_transit_1d`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(fb_public_transit_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the SafeGraph away from home indicator (`full_time_work_prop_7dav`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(away_from_home_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the SafeGraph bar visits indicator (`bars_visit_prop`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(bar_visits_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
```
##### Examples
We can plot some of the counties where rises in the SafeGraph restaurants visits indicator (`restaurants_visit_prop`) lead rises in cases.
```{r results="hide", echo=FALSE, warning = FALSE}
...
plot(as.factor(restaraunt_visits_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days")
```
...
## Performance
Performance metrics over all counties over all time for each signal
...
recall_precision_time_point_df = data.frame("Indicator" = indicator_name_list_time_point, "Per_Time_Point_Recall" = as.numeric(r_p_list_time_point[[1]]), "Per_Time_Point_Precision" = as.numeric(r_p_list_time_point[[2]]))


recall_precision_rise_point_df['F1 Score'] = 2 * (recall_precision_rise_point_df$Per_Rise_Point_Recall * recall_precision_rise_point_df$Per_Rise_Point_Precision) / (recall_precision_rise_point_df$Per_Rise_Point_Recall + recall_precision_rise_point_df$Per_Rise_Point_Precision)

recall_precision_time_point_df['F1 Score'] = 2 * (recall_precision_time_point_df$Per_Time_Point_Recall * recall_precision_time_point_df$Per_Time_Point_Precision) / (recall_precision_time_point_df$Per_Time_Point_Recall + recall_precision_time_point_df$Per_Time_Point_Precision)
```
...
```
#### Recall and Precision Per Time Point
```{r echo=FALSE, warning = FALSE, rows.print = 15}
recall_precision_time_point_df
```

## Conclusions

In this DAP analysis, we report a novel methodology for identifying substantial increases in cases and indicators and quantifying indicator success in anticipating case rises. Our methodology is simple and interpretable and provides several examples across various counties and time periods where indicator signals (e.g., doctors visits) can anticipate case rises. Together, our results suggest that doctors visits and the quidel antigen signals perform best in anticipating case rises, and that generally indicators most frequently lead case rises by over a week.
...
<!-- plot(as.factor(fb_test_positivity_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days") -->
<!-- ``` -->
...
<!-- plot(as.factor(fb_mask_frequencies), ylab="Occurrences", xlab="Days", main="Frequencies of 'leadingness' in days") -->
<!-- ``` -->
