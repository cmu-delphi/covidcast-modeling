...
get_and_prepare_signals <- function(cases_deaths, start_day, end_day, indicator_source, indicator_signal, 
                                    case_death_threshold, indicator_threshold, geo_type = "county") {
  # Get cases and indicator
  if(cases_deaths == "case")
  {
    cases = covidcast_signal(data_source = "usa-facts", signal = "confirmed_7dav_incidence_num",
                             start_day = start_day, end_day = end_day,
                             geo_type = geo_type)
  } else {
    cases = covidcast_signal(data_source = "usa-facts", signal = "deaths_7dav_incidence_num",
                             start_day = start_day, end_day = end_day,
                             geo_type = geo_type)
  ...
  large_case_geos = names(case_list)[which(sapply(case_list, function(a) a %>% summarize(sum(value))) > case_death_threshold)]
  ...
  if(cases_deaths == "case")
  {
    ...
    return (list("deaths" = case_list[large_geos], "indicator" = indicator_list[large_geos]))
  }
}
...
get_increase_points <- function(case_list, indicator_list, local_bandwidth=12,
                                local_quantile_threshold=0.75, local_threshold=0.2, local_period=0)
{
  ...
  for(i in 1:length(case_list))
  {
    ...
    if(length(which(merged_df$case_value == 0))/length(merged_df$case_value) > 0.8)
      next
    increasing_points_list_case_signal = calculate_increasing_points(merged_df$case_value, bandwidth=local_bandwidth, 
                                                                     quantile_threshold=local_quantile_threshold, 
                                                                     threshold=local_threshold, period=local_period)
    increasing_points_list_indicator_signal = calculate_increasing_points(merged_df$ind_value, bandwidth=local_bandwidth, 
                                                                          quantile_threshold=local_quantile_threshold, 
                                                                          threshold=local_threshold, period=local_period)
...
get_county_level_precision_recall <- function(final_cases_indicator_list, min_window=3, max_window=14)
{
  ...
  precision_recall_list=lapply(final_cases_indicator_list, function(x){
    #case_dates=as.Date(x$time_value[-c(1:min_window)])
    #indicator_dates = as.Date(x$time_value)
    #indicator_dates= as.Date(indicator_dates[-c((length(indicator_dates) - min_window+1): length(indicator_dates))])
    #print(x$geo_value[1])
    #case_rise_dates=x$time_value[which(x$case_rise_point == 1)]
    #indicator_rise_dates=x$time_value[which(x$indicator_rise_point == 1)]
    ...
    #random
    random_cases_preceded_by_indicators=get_cases_preceding_indicators(x$time_value[which(x$case_rise_point==1)], x$time_value[which(x$random_guesser==1)])
    random_each_indicator_rise_precedes_case_rise=get_indicator_preceding_cases(x$time_value[which(x$random_guesser==1)], x$time_value[which(x$case_rise_point==1)])

    #positive first derivative cases
    pos_case_first_deriv_cases_preceded_by_indicators = get_cases_preceding_indicators(x$time_value[which(x$case_rise_point==1)], x$time_value[which(x$case_first_deriv_guesser==1)])
    pos_case_first_deriv_indicator_rise_precedes_case_rise=get_indicator_preceding_cases(x$time_value[which(x$case_first_deriv_guesser==1)],x$time_value[which(x$case_rise_point==1)])
    
    #positive first derivative indicator
    pos_ind_first_deriv_cases_preceded_by_indicators = get_cases_preceding_indicators(x$time_value[which(x$case_rise_point==1)],x$time_value[which(x$indicator_first_deriv==1)])
    pos_ind_first_deriv_indicator_rise_precedes_case_rise = get_indicator_preceding_cases(x$time_value[which(x$indicator_first_deriv==1)],x$time_value[which(x$case_rise_point==1)])
    ...
    df=data.frame(County_Code=x$geo_value[1], 
               ...
               Num_Times_Case_Rise_Preceded_by_Indicator=sum(cases_preceded_by_indicators),
               Num_Times_Indicator_Rise_Precedes_Case_Rise=ifelse(length(each_indicator_rise_precedes_case_rise)>0,sum(each_indicator_rise_precedes_case_rise),0),
               Random_Precision_Denom=length(random_cases_preceded_by_indicators),
               Random_Recall_Denom=length(random_each_indicator_rise_precedes_case_rise),
               Positive_Case_Deriv_Precision_Denom=length(pos_case_first_deriv_cases_preceded_by_indicators),
               Positive_Case_Deriv_Recall_Denom=length(pos_case_first_deriv_indicator_rise_precedes_case_rise),
               Positive_Ind_Deriv_Precision_Denom=length(pos_ind_first_deriv_cases_preceded_by_indicators),
               Positive_Ind_Deriv_Recall_Denom=length(pos_ind_first_deriv_indicator_rise_precedes_case_rise),
               Random_Num_Times_Case_Rise_Preceded_by_Indicator=sum(random_cases_preceded_by_indicators),
               Random_Num_Times_Indicator_Rise_Precedes_Case_Rise=ifelse(length(random_each_indicator_rise_precedes_case_rise)>0,sum(random_each_indicator_rise_precedes_case_rise),0),
               Positive_Case_Deriv_Num_Times_Case_Rise_Preceded_by_Indicator=sum(pos_case_first_deriv_cases_preceded_by_indicators),
               Positive_Case_Deriv_Num_Times_Indicator_Rise_Precedes_Case_Rise=ifelse(length(pos_case_first_deriv_indicator_rise_precedes_case_rise)>0, sum(random_each_indicator_rise_precedes_case_rise),0),
               Positive_Ind_Deriv_Num_Times_Case_Rise_Preceded_by_Indicator=sum(pos_ind_first_deriv_cases_preceded_by_indicators),
               Positive_Ind_Deriv_Num_Times_Indicator_Rise_Precedes_Case_Rise=ifelse(length(pos_ind_first_deriv_indicator_rise_precedes_case_rise)>0, sum(pos_ind_first_deriv_indicator_rise_precedes_case_rise),0),
               Leading_Indicator_Precision=sum(cases_preceded_by_indicators)/length(cases_preceded_by_indicators),
               Leading_Indicator_Recall=ifelse(length(each_indicator_rise_precedes_case_rise)>0, sum(each_indicator_rise_precedes_case_rise)/length(each_indicator_rise_precedes_case_rise),NA),
               Random_Precision=sum(random_cases_preceded_by_indicators)/length(random_cases_preceded_by_indicators),
               Random_Recall=sum(random_each_indicator_rise_precedes_case_rise)/length(random_each_indicator_rise_precedes_case_rise),
               Positive_Case_Deriv_Precision=sum(pos_case_first_deriv_cases_preceded_by_indicators)/length(pos_case_first_deriv_cases_preceded_by_indicators),
               Positive_Case_Deriv_Recall=sum(pos_case_first_deriv_indicator_rise_precedes_case_rise)/length(pos_case_first_deriv_indicator_rise_precedes_case_rise),
               Positive_Ind_Deriv_Precision=sum(pos_ind_first_deriv_cases_preceded_by_indicators)/length(pos_ind_first_deriv_cases_preceded_by_indicators),
               Poisitve_Ind_Deriv_Recall=sum(pos_ind_first_deriv_indicator_rise_precedes_case_rise)/length(pos_ind_first_deriv_indicator_rise_precedes_case_rise),
               stringsAsFactors = F)
    df
    
  })
  precision_recall_df=rbindlist(precision_recall_list, use.names = T)
  global_precision_recall=data.frame(Precision_Leading_Indicator=sum(precision_recall_df$Num_Times_Case_Rise_Preceded_by_Indicator)/sum(precision_recall_df$Num_Case_Rises),
                                        Recall_Leading_Indicator=sum(precision_recall_df$Num_Times_Indicator_Rise_Precedes_Case_Rise)/sum(precision_recall_df$Num_Indicator_Rises),
                                        Precision_Random=sum(precision_recall_df$Random_Num_Times_Case_Rise_Preceded_by_Indicator)/sum(precision_recall_df$Random_Precision_Denom),
                                        Recall_Random=sum(precision_recall_df$Random_Num_Times_Indicator_Rise_Precedes_Case_Rise)/sum(precision_recall_df$Random_Recall_Denom),
                                        Precision_First_Case_Deriv=sum(precision_recall_df$Positive_Case_Deriv_Num_Times_Case_Rise_Preceded_by_Indicator)/sum(precision_recall_df$Positive_Case_Deriv_Precision_Denom),
                                        Recall_First_Case_Deriv=sum(precision_recall_df$Positive_Case_Deriv_Num_Times_Indicator_Rise_Precedes_Case_Rise)/sum(precision_recall_df$Positive_Case_Deriv_Recall_Denom),
                                        Precision_First_Ind_Deriv=sum(precision_recall_df$Positive_Ind_Deriv_Num_Times_Case_Rise_Preceded_by_Indicator)/sum(precision_recall_df$Positive_Ind_Deriv_Precision_Denom),
                                        Recall_First_Ind_Deriv=sum(precision_recall_df$Positive_Ind_Deriv_Num_Times_Indicator_Rise_Precedes_Case_Rise)/sum(precision_recall_df$Positive_Ind_Deriv_Recall_Denom),
                                        stringsAsFactors = F)
...
{
  #set.seed(1)
  #final_cases_indicator_list=fb_survey_cases_indicator_list
  ...
  # x = all_guessers[[1]]
  # 
  # sapply(x$time_value[which(x$case_rise_point==1)], function(case_increase_date) { 
  #      differences=as.integer(case_increase_date-x$time_value[which(x$indicator_rise_point==1)])
  #      min(differences[differences>0])
  # })
  # sapply(x$time_value[which(x$case_rise_point==1)], function(case_increase_date) { 
  #      differences=as.integer(case_increase_date-x$time_value[which(x$random_guesser==1)])
  #      min(differences[differences>0])
  # })
  # sapply(x$time_value[which(x$case_rise_point==1)], function(case_increase_date) { 
  #     differences=as.integer(case_increase_date-x$time_value[which(x$case_first_deriv_guesser==1)])
  #      min(differences[differences>0])
  # })
  # sapply(x$time_value[which(x$case_rise_point==1)], function(case_increase_date) { 
  #      differences=as.integer(case_increase_date-x$time_value[which(x$indicator_first_deriv==1)])
  #      min(differences[differences>0])
  # })
  # })
  # 
...
get_success_examples <- function(case_indicator_list, success_window, min_success_window) {
    ...
    if(case_points && indicator_points) {
      ...
      while(success && k <= length(case_dates)) {
        ...
        for (j in (1:length(indicator_dates))) {
          if(case_dates[k] - indicator_dates[j] <= success_window && case_dates[k] - indicator_dates[j] > min_success_window) {
            success = TRUE
          }
        }
        k = k+1
      ...
      if (success) {
        ...
        while(success && k <= length(indicator_dates)) {
          ...
          for (j in (1:length(case_dates))) {
            if(case_dates[j] - indicator_dates[k] <= success_window && case_dates[j] - indicator_dates[k] > min_success_window) {
              success = TRUE
            }
          }
          k = k+1
...
calculate_increasing_points <- function(signal, bandwidth=10, quantile_threshold=0.75, threshold=0.2, period=5){
  smoothed_signal=sm(signal, bandwidth)
  first_deriv = get_signal_first_derivative(signal = signal, bandwidth = bandwidth)
