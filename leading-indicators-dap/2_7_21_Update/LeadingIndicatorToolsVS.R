...
get_and_prepare_signals <- function(cases_deaths, start_day, end_day, indicator_source, indicator_signal, 
  ...
  #fb_survey = get_and_prepare_signals(cases_deaths="cases", 
...
generate_competitors_get_scores<-function(final_cases_indicator_list)
{
  ...
  scores=vector("list", length(final_cases_indicator_list))
  names(scores)<-unlist(lapply(final_cases_indicator_list, function(x) x$geo_value[1]))
