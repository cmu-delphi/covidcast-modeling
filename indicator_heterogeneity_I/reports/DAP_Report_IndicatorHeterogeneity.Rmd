...
We extend the above analysis to multiple leads, from 5 days ahead to
20 days ahead.  We also split the data into two time ranges over which
...
Disappointingly, the models with the static sensorized estimates do best, while
the ones with the dynamically
sensorized estimates tend to do worse than with just the raw indicator (!).  
(This is disappointing because the static sensorized estimates are "cheating",
i.e., in sample, and the dynamic sensorized ones are more "realistic" because
they are computed in an online fashion).  Perhaps this can be viewed in
light of the fact that dynamic sensorization tends to degrade time-wise
correlations -- and this notion of "proper ordering across time" is important
for forecasting.  (Evidence of overfitting?)
