...
except for the "external hosptitalization validation".
...
In the following figure, we omit the 'static sensorized' correlations,
because the are the same as the raw timewise correlations (merely a
linear shift and rescaling within each location, which does not affect
the orderings).

TODO: Remove the static sensorize correlations, because they are the same as
the raw ones in principle.
...
What we find is more peculiar.  We find that the median pointwise differences
between time-wise rank correlations of the sensorized and the raw
indicator actually are positive.  But again, the spreads are large.

TODO: Replace with mean and see what happens.
...
```{r timewise-correlations-compare-mean, cache=TRUE, message=FALSE,warning=FALSE}
timewise_cors_list = vector('list', length(source_names))
for (ind_idx in 1:length(source_names)) {
  timewise_cor_fname = sprintf('results/13_timewise_cors_%s_%s_%s_%s_-41_0.RDS',
                               geo_level, source_names[ind_idx],
...
conclude that the sensorized indicators "help" the base model.  But
how do the sensorized indicators compare against each other?
We perform the predictive task where we train each model with only
the sensorized indicator (without Cases/Deaths as a base covariate).
