...
using data from 10 weeks back to 1 week back.  **We find, unfortunately,
that even though this sensorization procedure  does improve geo-wise
correlations, it has several shortcomings, including degraded time-wise
correlations and some signs of overfitting.**
...
Overall, we find that geo-wise correlations improve; but time-wise correlations
degrade; and there is little impact on predictive ability, in terms of
including a sensorized indicator in the model with cases (or deaths).
...
that estimates are being produced in-sample.
...
We find, as previously discovered by Aaron in a Slack thread, that **the
dynamic sensorization can actually degrade time-wise correlations over moving
windows.** This is a very discouraging result, and motivates us to consider
some variant of static sensorization instead.

TODO: actually, not that bad, when you account for variability

```{r timewise-correlations, cache=TRUE}
timewise_cors_list = vector('list', length(source_names))
for (ind_idx in 1:length(source_names)) {
  timewise_cor_fname = sprintf('results/13_timewise_cors_%s_%s_%s_%s_-41_0.RDS',
...
TODO: just to be sure that it's not consistently worse by a little bit county by
county, we plot the differences.  still find that indeed, large variability even
in the differences.  so it's good
...
at lags of 7 days behind and 14 days behind.  The other three models
add the raw indicator, static sensorized indicator, and dynamic sensorized
indicator to the target (all three at the corresponding lags).
...
information" beyond the target.

TODO: also plot without cases
...
since October, and the consistent upward trajectory makes the prediction
task "easier".
...
plt = ggplot(
    ...
    x = "Date",
    y = "Scaled error (relative to strawman)"
  ) + theme_bw (
  ) + theme(
    legend.pos = "bottom",
    legend.title = element_blank()
...
We omit the "Targets" base model to reduce clutter in the model (it consistently
gets error close to zero, as seen in the previous section.
...
the HHS Hospitalization indicators.  The results are mixed:

* Hospitalizations, sensorized against Cases, when correlated against
  HHS Hospitalizations Confirmed (adult or pediatric), starts to do better
  than the raw Hospitalizations around mid-October.
* Hospitalizations, sensorized against Cases, when correlated against
  HHS Hospitalizations Confirmed or Suspected (adult or pediatric),
  does worse or about the same as the raw Hospitalizations indicator.
* Hospitalizations, sensorized against Deaths, fares worse than the raw
  Hospitalizations indicator on all fronts.

**In general, the fact that the geo-wise correlations tend to be degraded
suggest that we are running the risk of overfitting.**
...
correlations of the sensorized indicator against its intended target.  However,
the degradation of moving window time-wise correlations and our validation
procedures indicate that we may actually be overfitting to the target, despite
the fact that our time window, 10 weeks back to 1 week back, is both wide and
"airgapped" by a week from the date being predicted.  

It may be fruitful to consider a simple, fixed static sensorization; or
