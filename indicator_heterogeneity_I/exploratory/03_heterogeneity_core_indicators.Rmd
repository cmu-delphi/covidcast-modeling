...
end_day = NULL
cache_fname = 'cached_data/04_heterogeneity_core_indicators.RDS'
...
if (!file.exists(cache_fname)) {
  ...
  geo_values = suppressWarnings(covidcast_signal("usa-facts", "confirmed_cumulative_num",
                                max(df_target$time_value), 
                                max(df_target$time_value))) %>%
    filter(value >= case_num) %>% pull(geo_value)
  saveRDS(list(df_signals, df_target, df_deaths), cache_fname)
} else {
  cached_data = readRDS(cache_fname)
  df_signals = cached_data[[1]]
  df_target = cached_data[[2]]
