...
if (!file.exists(dv_sensorized_lm_fname)) {
  if (!PARCOMPUTE) {
    ...
    dv_sensorized_lm = parallel::mclapply(dv_grouped_list, function(df) {
        ~ broom::tidy(
          lm(cases_value ~ indicator_value, data = .x)
        ) %>% mutate (
          geo_value = unique(geo_value),
          sensorize_date = unique(sensorize_date),
...
for (outer_idx in 4:length(sensorize_time_ranges)) {
