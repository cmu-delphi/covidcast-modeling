...
plt = (ggplot(corr_mean_signal)
       + geom_line(aes(x=time_value,
                       ...
                       color=signal),
                   size=0.2)
       + ggtitle(paste('Median orrelation between GHT / 100 and Google Symptoms A+A', 
                       'across all states'))
       )
plt
...
res_all = res %>%
  ...
  mutate(dif12 = err1 - err2    # Compute differences
         , dif13 = err1 - err3, dif14 = err1 - err4
...
Differences in model fit are (very) significantly different, ignoring depedence structure at the moment. 
...
7-days ahead comparisons don't have so many low p-values. 14-days ahead comparisons look more significant across all alternative models. So it appears that these signals are useful for farther-future predictions, rather than near-future where number of cases is sufficient.
