...
We can see that cases + GS is the only model that outperforms cases alone. (Each alternative model is plotted separately with cases-only since errors are calculated on a shared-time basis. With all models together, a lot of dates are dropped and it causes major shifts in some error values.)

Oddly, the other alternative models, while not performing as well as cases + GS, outperformed the cases-only model according to the 7 and 14-day ahead median scaled errors above. There is some impact of calculating errors on different ranges of dates. Here dates shared between cases and cases + Symptoms + GHT, and cases and cases + GHT, cases-only error at 7 days ahead is 0.84 vs 0.92, or so.
