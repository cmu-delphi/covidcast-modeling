...
model_names = c("Cases", "Cases + Anosmia + Ageusia")
...
res_dif = res_all %>%
  ...
  mutate(lead = factor(lead, labels = paste(leads, "days ahead")),
         model = factor(model, 
                        labels = c("Cases vs Cases + Symptoms"))) 
...
Differences in model fit are (very) significantly different. 
...
```{r median error between cases only and cases + sypmtoms models by num days forecast ahead}
