...
As expected, there is clearly a positive relationship between GS A+A and GHT.
...
Clearly, missing GS values occur in the majority of states (36 of 51), with a large proportion corresponding to zeroes in GHT. Based on the previous plot, we consider any GHT value above approximately 100 to be "high" given the distribution of all GHT values.
...
model_names = c("Cases", "Cases + Symptoms", "Cases + GHT", 
                "Cases + Symptoms + GHT")
...
res_all = res %>%
  ...
  mutate(dif12 = err1 - err2, dif13 = err1 - err3, 
         dif14 = err1 - err4 # Compute differences
...
```{r prediction_setup over days ahead, include=FALSE}
# Transforms to consider, in what follows
trans = Id
inv_trans = Id
...
err_by_lead = res %>%
  drop_na() %>%                                       # Restrict to common time
  mutate(err1 = err1 / err0, err2 = err2 / err0, err3 = err3 / err0, err4 = err4 / err0) %>%  # Compute relative error to strawman model
  ungroup() %>%
  select(-err0) %>%
  pivot_longer(names_to = "model", values_to = "err",
               cols = -c(geo_value, time_value, lead)) %>%
  mutate(model = factor(model, labels = model_names[1:4])) %>%
  group_by(model, lead) %>%
...
We can see that models containing GS outperform cases-only. (Each alternative model is plotted separately with cases-only below since errors are calculated on a shared-time basis. With all models together, more dates are dropped and it causes shifts in some error values.)

The cases + GHT model outperforms cases-only slightly, especially at farther-ahead forecasts, but doesn't appear to improve models that already contain the GS signal. In fact, it appears that the cases + GS + GHT model is slightly worse than the cases + GS model, especially at slightly-ahead forecasts. 

```{r median error between cases only and cases + GS models by num days forecast ahead}
# Compute and plot median errors as function of number of days ahead.
err_by_lead = res %>%
  select(-err3, -err4) %>% 
  drop_na() %>%                                       # Restrict to common time
...
```{r median error between cases only and cases + GHT models by num days forecast ahead}
# Compute and plot median errors as function of number of days ahead.
err_by_lead = res %>%
  select(-err2, -err4) %>% 
  drop_na() %>%                                       # Restrict to common time
  mutate(err1 = err1 / err0, err2 = err3 / err0) %>%  # Compute relative error to strawman model
  ungroup() %>%
  select(-err0, -err3) %>%
  pivot_longer(names_to = "model", values_to = "err",
               cols = -c(geo_value, time_value, lead)) %>%
  mutate(model = factor(model, labels = model_names[c(1,3)])) %>%
  group_by(model, lead) %>%
  summarize(err = median(err)) %>% 
  ungroup()
...
```{r median error between cases only and cases + GS + GHT models by num days forecast ahead}
# Compute and plot median errors as function of number of days ahead.
err_by_lead = res %>%
  select(-err2, -err3) %>% 
  drop_na() %>%                                       # Restrict to common time
  mutate(err1 = err1 / err0, err2 = err4 / err0) %>%  # Compute relative error to strawman model
  ungroup() %>%
  select(-err0, -err4) %>%
  pivot_longer(names_to = "model", values_to = "err",
               cols = -c(geo_value, time_value, lead)) %>%
  mutate(model = factor(model, labels = model_names[c(1,4)])) %>%
  group_by(model, lead) %>%
  summarize(err = median(err)) %>% 
  ungroup()
