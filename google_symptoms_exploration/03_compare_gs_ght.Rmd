...
```{r absolute errors between all models by num days forecast ahead}
...
err_by_lead = res %>%
  ...
  pivot_longer(names_to = "model", values_to = "err",
               cols = -c(geo_value, time_value, lead)) %>%
  mutate(model = factor(model, labels = c("Strawman", model_names[1:4]))) %>%
  group_by(model, lead) %>%
...
ggplot(err_by_lead, aes(x = lead, y = err)) + 
  ...
  scale_color_manual(values = c("gray", "black", ggplot_colors)) +
  ...
  labs(title = "Forecasting errors by number of days ahead",
       ...
       x = "Number of days ahead", y = "Median error") +
  theme_bw() + theme(legend.pos = "bottom", legend.title = element_blank())
```
...
We can see that models containing GS outperform cases-only. The cases + GHT model outperforms cases-only slightly, especially at farther-ahead forecasts, but doesn't appear to improve models that already contain the GS signal. In fact, it appears that the cases + GS + GHT model is slightly worse than the cases + GS model, especially at slightly-ahead forecasts. 
