...
We can see that during some time periods, median scaled error for the cases + google-symptoms (anosmia, ageusia, A+A) model is more variable than other signals at county level.
...
```{r, warning=FALSE, message=FALSE, cache=TRUE}
##### Analysis #####
model_names = c("Cases", "Cases + Doctor Visits", "Cases + Survey Data:%COVID-like Illness",  "Cases + Survey Data: %COVID-like Illness in local community",
                "Cases + Hospitalization Admissions", "Cases + Google-symptoms:Anosmia", "Cases + Google Symptoms:Ageusia",
...
ggplot(err_by_lead, aes(x = lead, y = err)) + 
  ...
  labs(title = "Forecasting errors by number of days ahead",
       subtitle = sprintf("Over all counties using smoothed signals"),
