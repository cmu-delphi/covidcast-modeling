...
sources = c("doctor-visits", "fb-survey", "fb-survey", 
            "hospital-admissions", "ght")
signals = c("smoothed_adj_cli", "smoothed_cli", "smoothed_hh_cmnty_cli", 
            "smoothed_adj_covid19", "smoothed_search")
names = c("Doctor visits", "Facebook CLI", "Facebook CLI-in-community", 
          "Hospitalizations", "Google trends")
...
prefixes = c("dv", "fb_cli", "fb_cli_com", "hm", "ght", "anosmia", "ageusia", "aa")
...
Calculating median scaled error over all dates and all counties for a given model, we see that adding Ageusia as a predictor do not reduce the median scaled error much compared to a cases-only model.However, Anosmia search volume do help add predictive power.
...
We can see that models containing GS outperform other models including Case-only and Cases + other signals at MSA level. Errors are calculated on a shared-time basis. With all models together, more dates are dropped and it causes shifts in some error values. In fact, it shows that the prediction of cases + GS model is of high quality though with low geographical coverage. Compared with county level result, the Cases + Anosmia model outperforms the other two models containing GS which is probably because Anosmia search volume has better coverage than that of Ageusia. However, it might worth further exploration why Cases + Anosmia outperform Cases + Anosmia & Ageusia.