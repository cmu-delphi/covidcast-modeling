...
def static_sensorization_for_rawsum(setn, api_df_dict, gs_df):
    """
    Statis sensorization for google symptom sets indicators which are
    created based on raw sums
    ...
    rawsum_df = pd.DataFrame(columns=["date", "geo_id", "SetName", 
                                      "coef", "intercept", "predicted", 
                                      "raw", "cases"])
    j = 0
    ...
    for county in geo_set:
        ...
        local_df = gs_df.loc[gs_df["geo_id"] == county]
        X = local_df[setn].values.reshape([local_df.shape[0], 1])   
        ...
        for i in range(7, X.shape[0]):
            ...
            result.append(X[i:i+1, :][0][0])
            result.append(y[i])
            rawsum_df.loc[j] = result
            j+=1

    
    return rawsum_df

result = static_sensorization_for_rawsum(args.setn, api_df_dict, gs_df)

result.to_csv("./output/%s_rawsum_%s_sensorization_12_14.csv"%(args.setn, args.fillmissingness), index=False)