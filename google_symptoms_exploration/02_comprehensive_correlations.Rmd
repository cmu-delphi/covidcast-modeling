...
For the signal that is most sparselay available, the number of 
...
plt
```
```{r top_correlations_plot, echo = TRUE}
plt = (ggplot(plot_cor_df)
       + geom_line(aes(x=time_value,
                       y=value,
                       group=factor(signal)),
                   data=plot_cor_df %>% filter (
                      signal %in% top_cor_signals$signal,
                      !signal %in% top_cor_signals$signal[1:5],
                   ),
                   color='darkorange',
                   size=0.1)
       + geom_line(aes(x=time_value,
       ...
       + theme(axis.text.x = element_text(angle = 45))
       + ggtitle(paste(
                'Correlation between cases and symptom search volumes\n',
                'Only top 20 symptoms with largest all-time max corr plotted',
                sep='\n'))
       )
plt
```
...
top_min_cor = cor_df %>% group_by (
    ...
    abs(value) == abs(min(value, na.rm=TRUE)),
  ) %>% ungroup (
  ) %>% arrange(
    abs(value),
  ) %>% head (
...
```{r top_cor_symptoms, echo = TRUE}
print('Five symptoms with greatest all-time high correlation:')
print(top_cor_signals$signal[1:5])
print('Next fifteen:')
print(top_cor_signals$signal[6:20])
```

TODO:

* [ ] Look at the "consistently ok" ones - top 5 min
* [ ] Understand why there are some symptoms that fall in the top correlation 
  in July-ish, but don't fall into the top 20...
* [ ] Top 5: Group by location, aggregate over time, produce chloropeth?
* [ ] Table that lists symptom, summary statistics of correlation, 
