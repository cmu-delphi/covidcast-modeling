...
The difference between this analysis and the previous county level predictive power comparison is that, here we consider a larger geographic scope. The intersection of counties available for DV, FB CLI and FB CLI-in-community from 2020-05-06 to 2020-10-08 is considered. Counties with no data reported by GS indicators are assigned with a value 0. 


### Getting data
```{r, echo = FALSE, message = FALSE, warning=FALSE, cache=TRUE}
# Fetch the following sources and signals from the API 
sources = c("doctor-visits", "fb-survey", "fb-survey", "google-symptoms",
            "google-symptoms", "google-symptoms")
signals = c("smoothed_adj_cli", "smoothed_cli", "smoothed_hh_cmnty_cli", 
            "anosmia_smoothed_search", "ageusia_smoothed_search", "sum_anosmia_ageusia_smoothed_search")
names = c("Doctor visits", "Facebook CLI", "Facebook CLI-in-community", 
          "Google Symptoms Anosmia Search", "Google Symptoms Ageusia Search",
          "Google Symptoms A+A Search")
...
# Consider the complete intersection of geo_values except for GS indicators
# Assign zeros to GS indicators if there is no data reported
case_num = 200
...
geo_values_complete = intersect(geo_values, geo_values_complete)
```


```{r, warning=FALSE, message=FALSE}
# Fill missing geo value for google symptoms signals
for (i in 4:6){
  fill_index_df = df_signals[[i]] %>% group_by(time_value) %>%
    summarize(geo_value = geo_values_complete) %>% ungroup()
  df_signals[[i]] = full_join(df_signals[[i]], fill_index_df, by = c("geo_value", "time_value"))
  df_signals[[i]]$value[is.na(df_signals[[i]]$value)] = 0
}
```
...
# Re-scale all the signals to bring them down to proportions (between 0 and 1)
prefixes = c("dv", "fb_cli", "fb_cli_com", "anosmia", "ageusia", "aa")
...
colnames( df_cases ) = sub("^value", "case", colnames( df_cases ))

combined_df = df_cases
...
for (i in 1:length(signals)) {
  ...
  # Rename columns
  colnames( df_signals[[i]] ) = sub("^value", prefixes[i], colnames( df_signals[[i]] ))
  
  # Make one big matrix by joining these three data frames
  combined_df = full_join(combined_df, df_signals[[i]], by = c("geo_value", "time_value"))
}
```
...
model_names = c("Cases", "Cases + DV", "Cases + FB-CLI",  "Cases + FB-CLI-community",
                "Cases + GS-Anosmia", "Cases + GS-Ageusia",
...
for (i in 1:length(leads)) { 
  ...
  res_list[[i]] = combined_df %>% 
    ...
    mutate(err0 = as.double(NA), err1 = as.double(NA), err2 = as.double(NA), 
           ...
           err6 = as.double(NA), err7 = as.double(NA), lead = lead) 
...
We can see that during some time periods, median scaled error for the cases + google-symptoms (anosmia, ageusia, A+A) model is more variable than other signals at county level and is at the same level as that of case-only model. 

```{r compare_media_err_over_all_time_and_counties}
...
Calculating median scaled error over all dates and all counties available for DV, FB CLI and FB CLI-in-community for a given model, we see that adding symptoms as a predictor does not reduce th median scaled error much compared to a cases-only model.
...
comparison_names = paste("Cases vs", model_names[2:7])
...
```{r calc p-values from sign test across all counties for each date}
...
ggplot(res_dif %>% 
  ...
  facet_wrap(vars(lead, model), ncol = 3) + 
  labs(x = "P-value", y = "Count") +
  theme_bw() + theme(legend.pos = "none")
```

If the differences between models were *not* significant, we'd expect the distribution of p-values to be uniform. That is distinctly not what we see here. Model differences at both 7 and 14 days ahead have elevated (compared to expected) numbers of low p-values, suggesting significance. 
...
```{r prediction_setup over days ahead, warning=FALSE, message=FALSE}
# Set how far to forecast ahead.
leads = 5:20
lags = 1:2 * -7
rm(lead)
...
```{r, warning=FALSE, message=FALSE, cache=TRUE}
##### Analysis #####
model_names = c("Cases", "Cases + Doctor Visits", "Cases + Survey Data:%COVID-like Illness",
                "Cases + Survey Data: %COVID-like Illness in local community",
                "Cases + Google-symptoms:Anosmia", "Cases + Google Symptoms:Ageusia",
                "Cases + Google Symptoms:Anosmia+Ageusia")
...
```{r median error between all models by num days forecast ahead, eval=TRUE}
...
err_by_lead = res %>%
  ...
  mutate(err1 = err1 / err0, err2 = err2 / err0, err3 = err3 / err0, err4 = err4 / err0,
         err5 = err5 / err0, err6 = err6 / err0, err7 = err7 / err0) %>%  # Compute relative error to strawman model
  ungroup() %>%
  select(-err0) %>%
  pivot_longer(names_to = "model", values_to = "err",
               cols = -c(geo_value, time_value, lead)) %>%
  mutate(model = factor(model, labels = model_names[1:7])) %>%
  group_by(model, lead) %>%
...
ggplot(err_by_lead, aes(x = lead, y = err)) + 
  ...
  labs(title = "Forecasting errors by number of days ahead",
       subtitle = sprintf("Over all counties with more than 500 cumulative cases \n"),
...
We can see that models containing GS outperform the Cases only model and the Cases + GS A+ A model is as competitive as the Cases + DV signals. However, the GS related models do not outperform Cases + FB CLI and Cases + FB CLI-in-community models. Errors are calculated on a shared-time basis. With all models together, more dates are dropped and it causes shifts in some error values. In fact, compared with the results we got previously considering counties available for GS indicators only, it shows that the prediction of cases + GS model is of high quality in regions with data reported and is still valuable for a larger geographical scope. 
