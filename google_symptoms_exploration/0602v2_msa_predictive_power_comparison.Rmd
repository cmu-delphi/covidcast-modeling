...
## MSA Level Analysis
The difference between this analysis and the previous MSA level predictive power comparison is that, here we consider a larger geographic scope. The intersection of MSAs available for DV, FB CLI and FB CLI-in-community from 2020-05-06 to 2020-10-08 is considered. MSAs with no data reported by GS indicators are assigned with a value 0. 
...
# Get geo values that appear DV, FB CLI, FB CLI-in-community
...
# Filter to complete MSAs, transform the signals, append 1-2 week lags to 
...
model_names = c("Cases", "Cases + DV", "Cases + FB-CLI",  "Cases + FB-CLI-community",
                "Cases + GS-Anosmia", "Cases + GS-Ageusia", "Cases + Anosmia+Ageusia")
...
```{r compare_media_err_over_all_time_and_msas}
...
Calculating median scaled error over all dates and all counties for a given model, we see that adding Ageusia as a predictor do not reduce the median scaled error much compared to the cases-only model.However, they do help add predictive power especially for 7 days ahead. The cases + Anosmia and Ageusia model is as competitive as the Cases + DV model.
...
```{r calc p-values from sign test across all msas for each date}
...
Similar as what we got at county level, 14-day ahead comparisons look *more* significant across all alternative models. So it appears that additional predictors, especially GS, are useful for farther-future predictions, rather than near-future where number of cases is largely sufficient.
...
ggplot(err_by_lead, aes(x = lead, y = err)) + 
  ...
  labs(title = "Forecasting errors by number of days ahead",
       subtitle = sprintf("Over all MSAs using smoothed signals"),
       x = "Number of days ahead", y = "Median scaled error") +
  theme_bw() + theme(legend.pos = "bottom", legend.title = element_blank()) +
  guides(color = guide_legend(nrow = 3))
```

We got pretty simialr result as what we see in the county level analysis. The models containing GS outperform the Cases only model and the Cases + GS A+ A model is as competitive as the Cases + DV signals. And Cases + FB CLI and Cases + FB CLI-in-community models largely outperform other models. 